...
async function getcategories() {
  ...
  const data = await (
    await fetch("http://jservice.io/api/categories?count=100")
  ).json();

  //Fetches 9 categories
  const cluePromises = data.map((clue) =>
    fetch(`http://jservice.io/api/category?id=${clue.id}`).then((req) =>
      req.json()
    )
  );
  ...
  //trim the categories
  for (let i = 0; i < 6; i++) {
    let numb = getRandomInt(categories.length);
    filteredcat.push(categories.splice(numb, 1)[0]);
  }
  ...
  for (let i = 0; i < 6; i++) {
    ...
    clue.clues.forEach((cl) => {
      if (
        boxes[i + 6].innerHTML < 100 &&
        (cl.value == 200 ||
          (cl.value == 100 && parseInt(cl.airdate.split("-")[0]) <= 2001))
      ) {
        boxes[i + 6].innerHTML = 200;
        let dailyDouble = null;

        boxes[i + 6].addEventListener(
          "click",
          () => {
            ...
            if (boxes.indexOf(box) == rand) {
              dailyDouble = parseInt(
                prompt(
                  "WELCOME TO THE DAILY DOUBLE\n The Category::" +
              ...
              while (
                isNaN(dailyDouble) ||
                dailyDouble < 5 ||
                (dailyDouble > 1000 && score < 1000)
              ) {
                dailyDouble = parseInt(
                  prompt(
                    "Please bet up to either " +
                      score +
                      " or $1000 and a min of $5"
                  )
                );
              }
            }
      ...
      if (
        boxes[i + 12].innerHTML < 100 &&
        (cl.value == 400 ||
          (cl.value == 200 && parseInt(cl.airdate.split("-")[0]) <= 2001))
      ) {
        boxes[i + 12].innerHTML = 400;
        let timer = 30;
        let dailyDouble = null;
        boxes[i + 12].addEventListener(
          "click",
          () => {
            console.log(cl.answer);
            let box = boxes[i + 12];
      ...
      if (
        boxes[i + 18].innerHTML < 100 &&
        (cl.value == 600 ||
          (cl.value == 300 && parseInt(cl.airdate.split("-")[0]) <= 2001))
      ) {
        boxes[i + 18].innerHTML = 600;
        let dailyDouble = null;
        let timer = 30;
        boxes[i + 18].addEventListener(
          "click",
          () => {
            console.log(cl.answer);
            let box = boxes[i + 18];
      ...
      if (
        boxes[i + 24].innerHTML < 100 &&
        (cl.value == 800 ||
          (cl.value == 400 && parseInt(cl.airdate.split("-")[0]) <= 2001))
      ) {
        boxes[i + 24].innerHTML = 800;
        let timer = 30;
        let dailyDouble = null;
        boxes[i + 24].addEventListener(
          "click",
          () => {
            console.log(cl.answer);
            let box = boxes[i + 24];
      ...
      if (
        boxes[i + 30].innerHTML < 100 &&
        (cl.value == 1000 ||
          (cl.value == 500 && parseInt(cl.airdate.split("-")[0]) <= 2001))
      ) {
        boxes[i + 30].innerHTML = 1000;
        let timer = 30;
        let dailyDouble = null;
        boxes[i + 30].addEventListener(
          "click",
          () => {
            console.log(cl.answer);
            let box = boxes[i + 30];
